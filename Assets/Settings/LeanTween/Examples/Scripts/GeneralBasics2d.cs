...
public class GeneralBasics2d : MonoBehaviour
{
    ...
    {
        // Setup
        GameObject avatarRotate = createSpriteDude(
            "avatarRotate",
            new Vector3(-2.51208f, 10.7119f, -14.37754f)
        );
        GameObject avatarScale = createSpriteDude(
            "avatarScale",
            new Vector3(2.51208f, 10.2119f, -14.37754f)
        );
        GameObject avatarMove = createSpriteDude(
            "avatarMove",
            new Vector3(-3.1208f, 7.100643f, -14.37754f)
        );
        ...
        // Scale Example
        LeanTween
            .scale(avatarScale, new Vector3(1.7f, 1.7f, 1.7f), 5f)
            .setEase(LeanTweenType.easeOutBounce);
        LeanTween
            .moveX(avatarScale, avatarScale.transform.position.x + 1f, 5f)
            .setEase(LeanTweenType.easeOutBounce); // Simultaneously target many different tweens on the same object

        // Move Example
        LeanTween
            .move(avatarMove, avatarMove.transform.position + new Vector3(1.7f, 0f, 0f), 2f)
            .setEase(LeanTweenType.easeInQuad);

        // Delay
        LeanTween
            .move(avatarMove, avatarMove.transform.position + new Vector3(2f, -1f, 0f), 2f)
            .setDelay(3f);

        // Chain properties (delay, easing with a set repeating of type ping pong)
        LeanTween
            .scale(avatarScale, new Vector3(0.2f, 0.2f, 0.2f), 1f)
            .setDelay(7f)
    ...
    GameObject createSpriteDude(string name, Vector3 pos, bool hasParticles = true)
    {
        ...
        ren.sprite = Sprite.Create(
            dudeTexture,
            new Rect(0.0f, 0.0f, 256.0f, 256.0f),
            new Vector2(0.5f, 0f),
            256f
        );
        ...
        if (hasParticles)
        {
            GameObject particles = (GameObject)
                GameObject.Instantiate(
                    prefabParticles,
                    Vector3.zero,
                    prefabParticles.transform.rotation
                );
            ...
            .delayedCall(
                ...
                () =>
                {
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        // Instantiate Avatar
                        GameObject dude = createSpriteDude(
                            "dude" + i,
                            new Vector3(-2.51208f, 7.100643f, -14.37754f),
                            false
                        ); //(GameObject)GameObject.Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation );
                        ...
                        LeanTween
                            .scale(dude, new Vector3(0.65f, 0.65f, 0.65f), 1f)
                            .setDelay(i * 0.2f)
