...
public class GeneralSequencer : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        seq.append(
            LeanTween.moveY(avatar1, avatar1.transform.localPosition.y + 6f, 1f).setEaseOutQuad()
        );
        ...
        seq.append(() =>
        {
            for (int i = 0; i < 50f; i++)
            {
                ...
                LeanTween
                    .rotateAround(cloud, Vector3.forward, 360f * 2, 3f * speedScale)
                    .setEaseOutCirc();
                LeanTween
                    .alpha(cloud, 0f, 3f * speedScale)
                    .setEaseOutCirc()
